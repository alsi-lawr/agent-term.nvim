...
describe("Given native agent hook installation", function()
	...
	it("When installing Codex hooks Then project-local hook config and script are written", function()
		...
		assert.is_not_nil(handler.command:match("%.agent%-term/context%.json"))
	...
	it(
		...
		function()
			...
			assert.are.equal("${CLAUDE_PROJECT_DIR}/.claude/hooks/agent_term_context.py", handler.args[1])
	...
	it(
		...
		function()
			...
			assert.is_not_nil(notifications[1].msg:match("not supported for `gemini`"))
		end
	)
end)
